...
const { buildGanttData } = require('../lib/ganttMissions');
...
router.get('/missions', async (req, res) => {
  ...
  if (profile.role === 'admin') {
    ...
    return res.render('missions-admin', {
      ...
      gantt: buildGanttData(missions || []),
      filtreAlt: alt || '', filtreMois: mois || '', error: null
    });
  }

  let query = db.from('missions').select('*').eq('alternant_id', profile.id).order('date_debut', { ascending: true });
  if (mois) query = query.lte('date_debut', mois + '-31').gte('date_fin', mois + '-01');
  const { data: missions } = await query;
  res.render('missions-alternant', {
    missions: missions || [], gantt: buildGanttData(missions || [], profile), filtreMois: mois || ''
  });
});

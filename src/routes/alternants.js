...
async function chargerFiche(db, id, moisParam) {
  ...
  const [{ data: congesValidees }, { data: heures }, { data: planningRows }] = await Promise.all([
    ...
    db.from('planning').select('date,type').eq('alternant_id', id).gte('date', monthStart).lte('date', monthEnd)
  ]);
...
router.get('/alternants', requireAdmin, async (req, res) => {
  ...
  res.render('alternants', { alternants: alternants || [], societes: societes || [], selected, error: null, showModal: false });
});
...
router.post('/alternants', requireAdmin, uploadPlanning.single('fichier_planning'), async (req, res) => {
  ...
  // 3) Envoi du mot de passe temporaire par e-mail (à connecter plus tard à un vrai service d'e-mail,
  // ex. Resend ou Supabase SMTP). Pour l'instant on log en console pour test local.
  console.log(`Compte créé pour ${email} — mot de passe temporaire : ${tempPassword}`);
...
router.post('/alternants/:id', requireAdmin, async (req, res) => {
  const { poste, date_debut, date_fin, societe_id } = req.body;
  await req.db.from('profiles').update({ poste, date_debut, date_fin, societe_id: societe_id || null }).eq('id', req.params.id);
  res.redirect('/alternants?voir=' + req.params.id);
});
